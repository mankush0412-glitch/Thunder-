...
def _resolve_fqdn_and_ssl():
    """
    Auto-detect FQDN from Render's RENDER_EXTERNAL_URL env var.
    Render automatically sets: RENDER_EXTERNAL_URL=https://your-app.onrender.com
    No need to set FQDN manually when deploying on Render.
    """
    render_url = os.getenv("RENDER_EXTERNAL_URL", "").strip().rstrip("/")
    if render_url:
        has_ssl = render_url.startswith("https://")
        fqdn = render_url.replace("https://", "").replace("http://", "")
        logger.info(f"Auto-detected FQDN from Render: {fqdn}")
        return fqdn, has_ssl

    # Manual override via env
    fqdn = os.getenv("FQDN", "").strip() or os.getenv("BIND_ADDRESS", "0.0.0.0")
    has_ssl = str_to_bool(os.getenv("HAS_SSL", "True"))
    return fqdn, has_ssl
...
class Var:
    ...
    # Auto-resolve FQDN — reads RENDER_EXTERNAL_URL automatically on Render
    _fqdn, _has_ssl = _resolve_fqdn_and_ssl()
    FQDN: str = _fqdn
    HAS_SSL: bool = _has_ssl
